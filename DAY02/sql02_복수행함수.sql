 ...
 * COUNT(), SUM(), AVG(), MIN/MAX(), STDDEV()표준편차, ...
 * ROLLUP, CUBE, RANK...
 */
-- COUNT()
...
--AVG 숫자형 평균
-- 컬럼에 널값 있으면 제외하고 계산하기 떄문에 잘못된 값 도출됨
-- 금액이나 수량을 계산하는 컬럼의 널값은 전처리 해줘야함
SELECT avg(salary)
  FROM employees;
...
--employees에서 부서와 직군별 급여 총액과 직원수를 출력하는데
--department_id가 30에서90사이이고, 부서직군별 급여총액이 20000달러 이상인 데이터만 보일것
...
-- PIVOT() 엑셀에 동일한 기능
-- PIVOT 안쓰고 각 달 별로 입사한 사원의 수를 표시. 12행
-- 각 입사일자에서 달만 추출
