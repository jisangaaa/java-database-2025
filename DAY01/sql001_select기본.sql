 ...
 */
 ...
 -- 03. 02번의 쿼리를 칼럼명을 변경해서 출력하시오. '직원번호','이름','성','이메일','입사일자' 로 변경하시오(107행)
 ...
 -- 05. employees 테이블의 이름과 성을 붙여서 full_name으로 출력, phone_number를 출력하시오.(107행)
  ...
  * 조건절 - WHERE
  */
 -- 연습. employees 테이블에서 employ_id가 110인 직원을 출력하세요.(1행)
 SELECT *
   FROM employees
 ...
 SELECT *
  ...
  /*
   * where절에 사용하는 연산자
   * ● = : equal. 타 프로그래밍 언어와 달리 =한개만 사용
   * ● !=, <> : not equal 같지않다.
   * ● >,>=,<,<= : 부등호
   * ● BETWEEN a AND b : 특정값이 a와 b 사이에 있다.(a이상 b이하에 포함되어있다)
   * ● IN(A,B,C) : 특정값이 a이거나b 이거나 c 중 하나다.
   * ● LIKE : SQL에서 문자열 비교시 특화된 연산자.
   * ● IS NULL, IS NOT NULL : NULL값을 검색할 때, NULL값이 아닌 걸 검색할 때
   * ● a AND b : a와 b 조건 모두를 만족해야 True
   * ● a OR b : a와 b 조건 하나라도 만족하면 True
   * ● NOT a : a가 아닌 조건, 반대조건
   */
  ...
  -- 07. employees에서 hire_date가 2005-01-01 이후 입사했고 salary가 10000이상인 직원을 출력하라.
  --      이름과 이메일,전화번호, 입사일자, 급여 출력하라.
  SELECT first_name
  		,last_name
  		,phone_number
  		,hire_date
...
-- departments 에서 department_id 가 50 이하인 부서 아이디와 부서명 데이터
-- employees에서 employee_id가 110에서 150 사이인 직원아이디와 직원명(first_name || last_name)합쳐서 출력하시오.
